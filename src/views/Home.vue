...
import PopTypeAdd from '../components/PopTypeAdd.vue'
...
export default {
  ...
  setup() {
    ...
    const getBillList = async () => {
      try {
        const { data } = await axios.get(`bill/dayByMonth?month=${state.currentTime}&typeId=${state.currentSelect.id ? state.currentSelect.id : ''}&pageIndex=${state.page}&pageSize=5`)
        if (state.refreshing) {
          state.list = []
          state.refreshing = false
        }
        const updatedData = data.records.map(obj => {
          const updatedBills = obj.bills.map(bill => {
            return {
              ...bill,
