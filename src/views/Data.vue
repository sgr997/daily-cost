<template>
  <div class="data">
    ...
    <div class="structure">
      ...
      <div class="content">
        <div class="item" v-for="item in totalType == 'expense' ? expense_data : income_data" :key="item.type.id">
          <div class="left">
            <div class="type">
              <span :class="{ expense: totalType == 'expense', income: totalType == 'income' }">
                <i class="iconfont" :class="item.type.id ? item.type.icon : 0" />
              </span>
              <span>{{ item.type.typeName }}</span>
            </div>
            <div class="progress">{{ Number((item.number / Number(totalType == 'expense' ? total_expense : total_income))
              * 100).toFixed(2) }}%</div>
          </div>
          <div class="right">
            <div class="percent">
...
export default {
  ...
  setup() {
    ...
    const getData = async () => {
      ...
      const { data: resData } = await axios.get(`/bill/month?month=${state.currentMonth}`)
      const list = resData.records.map(item => {
        return {
          type: item.type,
      ...
      const data = {
        totalEexpense: resData.totalExpense / 100,
        totalIncome: resData.totalIncome / 100,
        totalData: list
      }

      // 总收支
      state.total_expense = data.totalEexpense
      state.total_income = data.totalIncome

      // 过滤支出和收入
      state.expense_data = data.totalData.filter(item => item.payType == 1).sort((a, b) => b.number - a.number) // 过滤出账单类型为支出的项
      state.income_data = data.totalData.filter(item => item.payType == 2).sort((a, b) => b.number - a.number) // 过滤出账单类型为收入的项
      setPieChart()
    }
    ...
    const setPieChart = () => {
      if (window.echarts) {
        ...
        proportionChart.setOption({
          ...
          // 图例
          legend: {
            data: _data.map(item => item.type.typeName)
          },
          series: [
            {
              name: '支出',
